...
function name() {
  ...
  for (let i = 0; i < totalDrops; i++) {
    const drop = document.createElement("div");
    drop.classList.add("rain-letters");

    drop.textContent = letters[Math.floor(Math.random() * letters.length)];

    // Random duration for each letter
    ...
    drop.style.animationDuration = duration + "s"; // Injects the random speed
    drop.style.fontWeight = Math.random() > 0.5 ? "bold" : "normal";

    const warmHue = Math.random() * 20 + 20;
    drop.style.textShadow = `0 0 8px hsl(${warmHue}, 60%, 50%)`;
    drop.style.color = `hsl(${warmHue}, 70%, 60%)`;
    drop.style.zIndex = 99999;

    // Forces the letter to stick to its final animation frame (the bottom)
    drop.style.animationFillMode = "forwards";

    document.body.appendChild(drop);

    // CRITICAL: Remove the setTimeout delete rule so the letters stay at the bottom forever!
    // If you want them to clear out eventually (e.g., after 20 seconds), uncomment the line below:
